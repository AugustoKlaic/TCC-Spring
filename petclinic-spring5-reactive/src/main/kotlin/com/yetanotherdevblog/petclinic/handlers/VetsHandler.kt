...
class VetsHandler(val vetRepository: VetRepository,
    ...
    fun add(serverRequest: ServerRequest) = serverRequest.body(BodyExtractors.toFormData())
            .flatMap {
                formData ->
                val speciality = mutableSetOf<String>()
                if(formData.containsKey("dentistry")){
                    speciality.add("dentistry")
                }
                if(formData.containsKey("radiology")){
                    speciality.add("radiology")
                }
                if(formData.containsKey("surgery")){
                    speciality.add("surgery")
                }
